...
const LoginForm: FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        const userKategori = user.user?.kategori;
        ...
        if(user.success === true){
          ...
          if(user.message === "Account is inactive" && user.id){
            notification.info({
              message: "Kredensial Ditemukan!",
              description: "Silakan aktivasi akun anda terlebih dahulu!",
            });
            setTimeout(() => {
              window.location.href = `/ActivateAccount/${user.id}`; 
            }, 1000);
          }
          else {
              notification.error({
      ...
      <div className="w-full md:w-1/2 md:pr-4">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p className="text-slate-500 mb-2">
              <Link className="text-left" to="/resendActivation">
                 Resend Aktivasi
              </Link>
        ...
        <p className="text-slate-500 mt-4 text-center">Belum memiliki akun?&nbsp;
          <Link to="/SignUp" className="text-maintheme">
          <b>Sign Up Disini</b>
          </Link>
        </p>
        {loading && (
